...
- Add `pagerduty-integration` notification channels, paging through the organization's installed Logfire PagerDuty App instead of a per-channel Events API v2 routing key. The config takes an `install_id` (the PagerDuty App installation created by connecting PagerDuty in the Logfire UI) and the `service_id` of an approved service. No secret is stored in Terraform state; the backend resolves the service's integration key at send time. The existing `pagerduty` type is unchanged and keeps working.
