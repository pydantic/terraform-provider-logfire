...
- Calls to APIs a self-hosted instance does not have yet now fail with an actionable message naming the minimum Logfire release (and Helm chart where known) and the instance version when it reports one, instead of a bare 404. The provider docs gained a self-hosted compatibility table listing the minimum release per feature.
