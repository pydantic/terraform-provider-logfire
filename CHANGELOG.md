...
- Collection endpoints for newer APIs (API keys, Gateway providers, instance organizations) now turn a missing-route 404 into an actionable message naming the minimum Logfire release (and Helm chart where known), plus the instance version when it reports one. The provider docs gained a self-hosted compatibility table listing the minimum release per feature.
