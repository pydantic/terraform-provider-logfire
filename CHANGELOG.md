...
These changes need the first Logfire release after v2026-09-22.02, which sets SLO channels per tier alert and serves the schedules API. Do not upgrade the provider before your Logfire instance has that release. On an older release, an SLO create or update that sets `alerts.<tier>.channel_assignments` fails with an error that names this release: the older release ignores `alerts` on SLO writes, so the SLO is saved but its alerts do not notify the configured channels. `logfire_alert.channel_assignments` also works on older releases.
